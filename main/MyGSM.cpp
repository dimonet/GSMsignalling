...
bool MyGSM::WaitingAvailable()
{
  ...
  int i = 0;   
  while(i < AVAILABLE_TIMEOUT/50)                                     // Количество циклов считаем как: заданые таймаут в милисекундах делим на (задержку внутри цикла 40мс + задержку в методе IsAvailable 10мс)
  {  
