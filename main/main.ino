...
{   
  ...
  if (inTestMod)                                                                            // если включен режим тестирования и не тревога
    if (!isAlarm)
  ...
  if (isAlarm)                                                                              // если тревога и если прошло заданное время отключаем тревогу
  {
    ...
    if (!SenGas.IsTrig)
      if (GetElapsed(prAlarm) > cAlarm || prAlarm == 0)                                     // если тревога больше установленного времени и не надо сигнализировать о газе то выключаем светодиод о индикации тревоги
        StopAlarm();
  }
  ...
  {     
    ...
    {       
      ...
      if (mode == OutOfContrMod)                                                            // если кнопку нажали заданное количество для включение/отключения режима тестирования
      {
        if (countPressBtn == EEPROM.read(E_BtnOnContr))
        {
          countPressBtn = 0;  
          Set_OnContrMod(true);       
        }
      ...
      if (mode == OutOfContrMod)                                                            // если кнопку нажали заданное количество для включение/отключения режима тестирования
      {
        if (countPressBtn == EEPROM.read(E_BtnInTestMod))
        {
          countPressBtn = 0;            
          if (!isAlarm)                                                                     // если нет необходимости сигнализировать о тревоге
            PORTB &= ~(1 << AlarmLED);                                                      // выключаем светодиод тревоги
          PlayTone(sysTone, 250);                                                           // сигнализируем об этом спикером            
          InTestMod(!inTestMod);                                                            // включаем/выключаем режим тестирования     
        }
      }
      else
      // запрос баланса счета
      if (mode == OutOfContrMod)                                                             // если кнопку нажали заданное количество для запроса баланса счета
      {
        if (countPressBtn == EEPROM.read(E_BtnBalance))
        {
          countPressBtn = 0;  
          PlayTone(sysTone, 250);                                                            // сигнализируем об этом спикером                        
      ...
      if (mode == OnContrMod)      
      {
        if (countPressBtn == EEPROM.read(E_BtnOutOfContr))
        {
  ...
  if (EEPROM.read(E_IsGasEnabled))                                                        // если датчик газа/дыма включен
  {
    ...
    {       
      ...
      if (!SenGas.IsTrig)
        if (inTestMod) 
          PlayTone(sysTone, 100);                                                         // если включен режим тестирование и это первое срабатывание то сигнализируем спикером  
      SenGas.IsTrig = true;
...
{ 
  PORTB &= ~(1 << AlarmLED);
  PORTD &= ~(1 << boardLED);                            // во время ожидание перед установкой на охрану, что б не горел внутренний светодиод, выключаем его (он моргает в режиме тестирования)
  ...
  if (!inTestMod)
    if (EEPROM.read(E_BtnOutOfContr)==0)                // если не включен режим тестирования и отключена возможность снимать с охраны кнопкой 
...
{  
  if (SirEnabled)                                       // и сирена не отключена в конфигурации
    PORTD |= 1 << SirenGenerator;                       // включаем сирену через через транзистор или релье               
  isSiren = true; 
  prSiren = millis(); 
...
void StopSiren()
{
  PORTD &= ~(1 << SirenGenerator);                      // выключаем сирену через транзистор или релье
  isSiren = false;   
}
...
void StartAlarm()
{
  if (!isAlarm)                                         // если еще невклюена тревога то включаем ее (проверяем для того что бы не выполнять лишний раз метод)
  {
    ...
    PORTB |= 1 << AlarmLED;                             // сигнализируем светодиодом о тревоге
...
void StopAlarm()
{
  PORTB &= ~(1 << AlarmLED);                            // Если не надо сигнализировать о газе то выключаем светодиод о индикации тревоги 
  isAlarm = false;  
}
...
void PowerControl()                                                                       // метод для обработки событий питания системы (переключение на батарею или на сетевое)
{
  ...
  if (!inTestMod)
    if (!powCtr.IsBattPowerPrevious)
      if (powCtr.IsBattPower)                                                             // если предыдущий раз было от сети а сейчас от батареи (пропало сетевое питание 220v) и если не включен режим тестирования
        SendSms(&GetStrFromFlash(sms_BattPower), &NumberRead(E_NUM1_OutOfContr));         // отправляем смс о переходе на резервное питание         
   
  if (!inTestMod)
    if (powCtr.IsBattPowerPrevious)
      if (!powCtr.IsBattPower)                                                            // если предыдущий раз было от батареи a сейчас от сети (сетевое питание 220v возобновлено) и если не включен режим тестирования  
...
{
  PORTB |= 1 << AlarmLED;                                                                 // включаем светодиод тревоги  
  PORTD |= 1 << SirenGenerator;                                                           // включаем сирену        
  delay(timeSkimpySiren);                                                                 // кратковременный период на который включается сирена
  PORTD &= ~(1 << SirenGenerator);                                                        // выключаем сирену через релье  
  if (!isAlarm) 
    PORTB &= ~(1 << AlarmLED);                                                            // выключаем светодиод если нет необходимости сигнализировать о тревоге     
}
...
{ 
  if (gsm.NewSms)
  {
    ...
    { 
      ...
      {        
        if (!isAlarm)                                                                    // если нет необходимости сигнализировать о тревоге
          PORTB &= ~(1 << AlarmLED);                                                     // выключаем светодиод, который может моргать если включен тестовый режим
        PlayTone(sysTone, smsSpecDur); 
