...
void setup() 
{
 ...
 // debug.begin(19200);
  ...
  if (WDRebooted) 
  {
    gsm.Shutdown(false);                      // если произошла hard перезагрузка то выключаем gsm модуль    
  } 
  ...
  { 
    ...
    if (count == 100)
    {
        ...
        SoftReboot();                                 // перезагружаем устройство
    }
  }  
  ...
  SMSRebooted = EEPROM.read(E_SMSRebooted);             // читаем был ли последний раз перезагрузка программным путем 
  ...
  {
    if (SMSRebooted)                                       // если устройство запускается после аварийной перезагрузки
...
{ 
  if (gsm.NewSms)
  {
    ...
    { 
      ...
      {
        PlayTone(sysTone, smsSpecDur);
        EEPROM.write(E_SMSRebooted, true);                                               // записываем статус, что устройство перезагружается        
        ...
        HardReboot();                                                                    // перезагружаем устройство испульзуя WatchDog. После перезагрузки во время запуска устройства, gsm модуль тоже будет перезагружен
      }      
      else 
